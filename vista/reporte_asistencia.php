...
<div class="page-content">
    ...
    <h4 class="text-center text-secondary">LISTA DE ASISTENCIAS DE AFILIADOS</h4>

    <?php
    include "../modelo/conexion.php";
    $sql = $conexion->query(" select * from afiliado ");
    ?>
    ...
    <select required name="txtafiliado" class="input input__select mb-2">
    <option value="todos">Todos los afiliados</option>
        <?php
        while ($datos = $sql->fetch_object()) { ?>
        <option value="<?= $datos->id_afiliado ?>"><?= $datos->nombre . " " . $datos->apellido ?></option>
        <?php }
        ?>
    </select>
